...
export default function AkademikDashboard() {
  ...
  useEffect(() => {
    fetch('/api/auth/me')
      .then((r) => r.json())
      .then((data) => {
        if (!data.user || data.user.userType !== 'ACADEMIC') {
  ...
  function handleSuccess(petition: { trackingCode: string; konu: string }) {
    setShowForm(false);
    setSuccessMsg(`Başvurunuz başarıyla iletildi. Takip kodunuz: ${petition.trackingCode}`);
    fetchPetitions();
    ...
    <div className="page-wrapper">
      ...
      <div className="page-hero">
        <div className="page-hero-inner">
          ...
          <p>Hoş geldiniz, {user.ad} {user.soyad}. Başvurularınızı buradan takip edebilirsiniz.</p>
        </div>
      </div>
      ...
      <div className="main-content">
        ...
        <div className="stats-grid">
          ...
          <div className="stat-card">
            <div className="stat-icon">⏳</div>
            <div className="stat-value">{stats.beklemede}</div>
            <div className="stat-label">Beklemede</div>
          </div>
          <div className="stat-card">
            <div className="stat-icon">🔍</div>
            <div className="stat-value">{stats.incelemede}</div>
            <div className="stat-label">İncelemede</div>
          </div>
          <div className="stat-card">
            <div className="stat-icon">✅</div>
            <div className="stat-value">{stats.cevaplandi}</div>
